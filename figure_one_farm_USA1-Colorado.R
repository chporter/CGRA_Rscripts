...
#hdat<-as.numeric(substring(x$HDA[x$TNAM==tnam],5,8))

pyear <- floor(x$PDAT / 1000)  # truncate
pdoy <- x$PDAT %% 1000         # modulo function
pdat <- ISOdate(pyear,12,31) + pdoy * 24 * 3600

ayear <- floor(x$ADAT / 1000)
adoy <- x$ADAT %% 1000
adat <- ISOdate(ayear,12,31) + adoy * 24 * 3600

myear <- floor(x$MDAT / 1000)
mdoy <- x$MDAT %% 1000
mdat <- ISOdate(myear,12,31) + mdoy * 24 * 3600

nday_flo <- as.numeric(adat - pdat)
nday_mat <- as.numeric(mdat - pdat)
head(nday_flo)
head(nday_mat)
...
dir<-"Summary_Colorado_maize.jpg"
...
boxplot(x$CWAM~x$WSTA, xaxt="n",yaxt="n",las=1,las=1,at=c(1,2,3,4,5,6, 8,9,10,11,12),col=c("gray", "green","red", "blue","yellow", "pink", "green","red", "blue","yellow", "pink"))
#axis(1, at=1:11,labels=c("OXFX","V4XF","V5XF","V8XF","VOXF","VTXF","W4XF","W5XF","W8XF","WOXF","WTXF"), tck=0.01,las=2,cex.axis=1)
#axis(1, at=1:12,labels=c("Base","CanAM4","CAM4","HadAM3P"," MIROC5","NorESM1", "","CanAM4","CAM4","HadAM3P"," MIROC5","NorESM1"), tck=0.01,las=2,cex.axis=1)
axis(1, at=1:11, tck=0.01,labels=FALSE,las=2,cex.axis=cex)
axis(2, tck=0.01,las=1,cex.axis=cex)
...
boxplot(nday_flo~x$WSTA, xaxt="n",yaxt="n",las=1,las=1,at=c(1,2,3,4,5,6, 8,9,10,11,12),col=c("gray", "green","red", "blue","yellow", "pink", "green","red", "blue","yellow", "pink"))
#axis(1, at=1:11,labels=c("OXFX","V4XF","V5XF","V8XF","VOXF","VTXF","W4XF","W5XF","W8XF","WOXF","WTXF"), tck=0.01,las=2,cex.axis=1)
#axis(1, at=1:12,labels=c("Base","CanAM4","CAM4","HadAM3P"," MIROC5","NorESM1", "","CanAM4","CAM4","HadAM3P"," MIROC5","NorESM1"), tck=0.01,las=2,cex.axis=1)
axis(1, at=1:11, tck=0.01,labels=FALSE,las=2,cex.axis=cex)
axis(2, tck=0.01,las=1,cex.axis=cex)
...
boxplot(nday_mat~x$WSTA, xaxt="n",yaxt="n",las=1,las=1,at=c(1,2,3,4,5,6, 8,9,10,11,12),col=c("gray", "green","red", "blue","yellow", "pink", "green","red", "blue","yellow", "pink"))
#axis(1, at=1:11,labels=c("OXFX","V4XF","V5XF","V8XF","VOXF","VTXF","W4XF","W5XF","W8XF","WOXF","WTXF"), tck=0.01,las=2,cex.axis=1)
axis(1, at=1:12,labels=c("Base","CanAM4","CAM4","HadAM3P"," MIROC5","NorESM1", "","CanAM4","CAM4","HadAM3P"," MIROC5","NorESM1"), tck=0.01,las=2,cex.axis=1)
#axis(1, at=1:11, tck=0.01,labels=FALSE,las=2,cex.axis=cex)
axis(2, tck=0.01,las=1,cex.axis=cex)
...
#####to export the figure as a file
dir<-"Summary_Colorado_maize_weather.jpg"
jpeg(dir,res=300, width=7, height=11, unit="in")
#####
