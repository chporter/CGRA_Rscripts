...
x<-read.csv("summary_maize.csv",header=TRUE,sep=",")
...
x$nday_flo<-adat-pdat
x$nday_mat<-hdat-pdat
head(x$nday_flo)
head(x$nday_mat)
...
while (start <= num) {
  ...
  farm <- x[start:end, ]
  farm <- subset(farm, !is.na(HWAM) & HWAM != "" & HWAM != -99)
  if (nrow(farm) == 0) {
    start <- end + 1
    next
  }
  ...
  farmData <- data.frame(
    WSTA = farm[1,]$WSTA,
    HWAM = ave(farm$HWAM)[1],
    CWAM = ave(farm$CWAM)[1],
    nday_flo = ave(farm$nday_flo)[1],
    nday_mat = ave(farm$nday_mat)[1]
  )
  if (start == 1) {
    mergedAve <- farmData
  } else {
...
#####to export the figure as a file
dir<-"Summary_maize_all_weather.jpg"
jpeg(dir,res=300, width=7, height=11, unit="in")
#####
